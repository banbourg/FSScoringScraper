...
for (event, year, season, link) in google_link_list:
    ...
    if isu_name in link and (right_year in link or (str(year)[-2:] in link and isu_name != "gpf") or
                             (isu_name == "gpf" and str(year)[-2:]+str(year+1)[-2:] in link)):
...
# with open(WRITE_PATH + "dates.csv", "w") as out:
#     csv_out = csv.writer(out)
#     csv_out.writerow(["event", "season", "event_start_date"])
